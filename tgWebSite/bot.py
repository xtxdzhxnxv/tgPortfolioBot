...
# Эндпоинт для приема уведомлений от Telegram (сделали универсальный путь /webhook)
...
def webhook():
    bot.remove_webhook()
    # Привязываем Telegram к новому чистому адресу /webhook
    bot.set_webhook(url=RENDER_URL + '/webhook')
    return "Бот-портал работает через чистый вебхук!", 200
